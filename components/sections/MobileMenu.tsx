...
export default function MobileMenu({linkColor}: {linkColor?: string}) {
	...
	return (
		<div>
			{!open &&
        <button aria-label="open navigation" className="border border-solid px-3 py-1 rounded-full hover:opacity-75 sm:hidden" onClick={toggleOpen}>
			...
			{open && 
        <button aria-label="close navigation" className={"border border-solid px-3 py-1 rounded-full hover:opacity-75 sm:hidden"} onClick={toggleOpen}>
          ...
          <motion.ul 
          	...
          	exit="exit" 
          	variants={variants} 
          	className={` font-bold fixed px-8 l-0 r-0 h-screen w-full left-0 bg-gray-50/95 rounded-3xl mt-10 z-50 py-6 border-t`}>
